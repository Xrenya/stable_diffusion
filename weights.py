"""
Weight conversion utility for Stable Diffusion 1.5.

This module handles loading the official SD 1.5 checkpoint and mapping its parameter
names to our custom model architecture. The original checkpoint uses the naming
convention from CompVis/stable-diffusion, while our tutorial code uses more descriptive
names. This file is purely a mechanical key-mapping — no model logic lives here.

Reference: https://github.com/kjsman/stable-diffusion-pytorch/issues/7#issuecomment-1426839447
"""
...
def load_from_standard_weights(input_file: str, device: str) -> dict[str, torch.Tensor]:
    """
    Load the official SD 1.5 .ckpt file and convert its state_dict keys to match
    our model definitions (VAE Encoder, VAE Decoder, CLIP, UNet/Diffusion).

    Args:
        input_file: Path to the .ckpt file (e.g. 'v1-5-pruned-emaonly.ckpt').
        device: Device to load the weights onto ('cuda', 'cpu', 'mps').

    Returns:
        A dict with keys 'encoder', 'decoder', 'clip', 'diffusion', each containing
        a state_dict ready for model.load_state_dict().
    """
